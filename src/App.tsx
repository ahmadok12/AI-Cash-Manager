...
import { goalProgress, goalSaved, installmentSchedule, ledgerOutstanding, ledgerPrincipal, ledgerReceived, normalizePlanningState, PersonLedger, PersonLedgerEntry, SavingsEntry, SavingsGoal } from "./planning";
import { createLedgerPdf, ledgerPdfFile } from "./ledger-pdf";

type Direction = "IN" | "OUT";
type Source = "Voice" | "Chat" | "Manual" | "Opening" | "Transfer";
type Transaction = { id:number; amount:number; description:string; direction:Direction; action:string; date:string; time:string; source:Source; status:"Synced"|"Pending"|"Local"; walletId:string; transferId?:string; ledgerEntryId?:string };
...
export default function CashApp(){
  ...
  const [goalEntryDraft,setGoalEntryDraft]=useState({amount:"",direction:"ADD" as SavingsEntry["direction"],date:todayKey(),note:""});
  ...
  function importSheetChanges(){
    ...
    const imported=sheetConflict.sheetTransactions.map(t=>({...t,status:"Synced" as const,ledgerEntryId:ledgerEntries.find(entry=>entry.transactionId===t.id)?.id}));
  ...
  async function confirmDelete(){
    ...
    setTransactions(next);if(deleteTarget.ledgerEntryId)setLedgerEntries(prev=>prev.filter(entry=>entry.id!==deleteTarget.ledgerEntryId));setDeleteTarget(null);notify(deleteTarget.ledgerEntryId?"Wallet and person ledger entry deleted":"Transaction deleted");
  ...
  function openNewGoal(){setGoalDraft({name:"",targetAmount:"",targetDate:"",note:""});setGoalOpen(true)}
  function saveGoal(e:FormEvent){
    ...
    const goal:SavingsGoal={id:`goal-${Date.now()}`,name:goalDraft.name.trim(),targetAmount,targetDate:goalDraft.targetDate||undefined,note:goalDraft.note.trim()||undefined,createdAt:new Date().toISOString()};
    setGoals(prev=>[goal,...prev]);setGoalOpen(false);notify("Savings goal created");
  }
  function openGoalEntry(goal:SavingsGoal,direction:SavingsEntry["direction"]="ADD"){
    setGoalEntryTarget(goal);setGoalEntryDraft({amount:"",direction,date:todayKey(),note:""});
  }
  function saveGoalEntry(e:FormEvent){
    e.preventDefault();if(!goalEntryTarget)return;const amount=Number(goalEntryDraft.amount);if(!amount)return;
    const saved=goalSaved(savingsEntries,goalEntryTarget.id);if(goalEntryDraft.direction==="WITHDRAW"&&amount>saved){notify(`Only ${formatMoney(saved)} is reserved in this goal`);return}
    const unreserved=Math.max(0,totalBalance(transactions)-totalSaved);if(goalEntryDraft.direction==="ADD"&&amount>unreserved){notify(`Only ${formatMoney(unreserved)} is currently unreserved`);return}
    const entry:SavingsEntry={id:`saving-${Date.now()}`,goalId:goalEntryTarget.id,amount,direction:goalEntryDraft.direction,date:goalEntryDraft.date,note:goalEntryDraft.note.trim()||undefined};
    setSavingsEntries(prev=>[entry,...prev]);setGoalEntryTarget(null);notify(goalEntryDraft.direction==="ADD"?"Savings updated":"Withdrawal recorded");
  }

  function openNewLedger(){setLedgerDraft({personName:"",relation:"",mode:"TARGET",openingReceivable:"",installmentAmount:"",firstDueDate:"",note:""});setLedgerOpen(true)}
  function saveLedger(e:FormEvent){
    ...
    const ledger:PersonLedger={id:`person-${Date.now()}`,personName:ledgerDraft.personName.trim(),relation:ledgerDraft.relation.trim()||undefined,mode:ledgerDraft.mode,openingReceivable,installmentAmount:installmentAmount||undefined,firstDueDate:ledgerDraft.firstDueDate||undefined,note:ledgerDraft.note.trim()||undefined,createdAt:new Date().toISOString()};
    setPeopleLedgers(prev=>[ledger,...prev]);setLedgerOpen(false);setSelectedLedger(ledger);notify("Person ledger created");
  }
  function openPersonEntry(ledger:PersonLedger,kind:PersonLedgerEntry["kind"]){
    setSelectedLedger(ledger);setLedgerEntryDraft({kind,amount:"",date:todayKey(),walletId:selectedWallet.id,note:""});setLedgerEntryOpen(true);
  }
  async function savePersonEntry(e:FormEvent){
    ...
    const outstanding=ledgerOutstanding(selectedLedger,ledgerEntries);
    if(ledgerEntryDraft.kind==="RECEIVED"&&amount>outstanding){notify(`Outstanding amount is ${formatMoney(outstanding)}`);return}
    if(ledgerEntryDraft.kind==="LENT"&&amount>walletBalance(transactions,wallet.id)){notify(`Not enough balance · available ${formatMoney(walletBalance(transactions,wallet.id))}`);return}
    const now=new Date();const transactionId=Date.now();const entryId=`ledger-entry-${transactionId}`;const direction:Direction=ledgerEntryDraft.kind==="RECEIVED"?"IN":"OUT";
    ...
    const next=[transaction,...transactions];setTransactions(next);setLedgerEntries(prev=>[entry,...prev]);setLedgerEntryOpen(false);notify(direction==="IN"?"Installment received":"Money given and receivable updated");
  ...
  async function downloadLedger(ledger:PersonLedger){const {doc,filename}=await createLedgerPdf(ledger,ledgerEntries,profile,walletNames);doc.save(filename);notify("Ledger PDF downloaded")}
  async function shareLedger(ledger:PersonLedger){
    const {doc,filename}=await createLedgerPdf(ledger,ledgerEntries,profile,walletNames);const file=ledgerPdfFile(doc,filename);const text=`${ledger.personName}'s receivable ledger · Outstanding ${formatMoney(ledgerOutstanding(ledger,ledgerEntries))}`;
  ...
  return <main className="phone-shell">
    {tab==="home"&&<section className="screen home-screen">
      ...
      <button className="planning-button" onClick={()=>setTab("plans")}><PiggyBank/><span><strong>Savings & people ledgers</strong><small>{formatMoney(totalSaved)} saved · {formatMoney(totalReceivable)} receivable</small></span><ChevronRight/></button>
    ...
    {tab==="plans"&&<section className="screen plans-screen"><Header title="Plans & ledgers" back/>
      <section className="plans-hero"><div><small>FAMILY MONEY PLAN</small><h2>{planView==="goals"?formatMoney(totalSaved):formatMoney(totalReceivable)}</h2><p>{planView==="goals"?"Reserved toward your future goals":"Still to receive from people"}</p></div><span>{planView==="goals"?<PiggyBank/>:<HandCoins/>}</span></section>
      <div className="plan-tabs"><button className={planView==="goals"?"active":""} onClick={()=>setPlanView("goals")}><Target/> Savings goals</button><button className={planView==="people"?"active":""} onClick={()=>setPlanView("people")}><UsersRound/> People ledgers</button></div>
      {planView==="goals"&&<><div className="plans-heading"><div><small>SAVINGS GOALS</small><h2>What are you saving for?</h2></div><button onClick={openNewGoal}><Plus/> Add goal</button></div>{!activeGoals.length?<Empty icon={<PiggyBank/>} title="Create your first savings goal" text="Track money reserved for future assets, shopping, tours, or anything important."/>:<div className="goal-list">{activeGoals.map(goal=>{const saved=goalSaved(savingsEntries,goal.id);const progress=goalProgress(goal,savingsEntries);return <article className="goal-card" key={goal.id}><div className="goal-card-head"><span><Target/></span><div><strong>{goal.name}</strong><small>{goal.targetDate?`Target ${new Date(`${goal.targetDate}T12:00:00`).toLocaleDateString("en-PK",{month:"short",year:"numeric"})}`:"No target date"}</small></div><b>{Math.round(progress)}%</b></div><div className="goal-amount"><strong>{formatMoney(saved)}</strong><span>of {formatMoney(goal.targetAmount)}</span></div><div className="progress-track"><i style={{width:`${progress}%`}}/></div><div className="goal-actions"><button onClick={()=>openGoalEntry(goal,"WITHDRAW")} disabled={saved<=0}>Withdraw</button><button onClick={()=>openGoalEntry(goal,"ADD")}><Plus/> Add saving</button></div></article>})}</div>}</>}
      {planView==="people"&&<><div className="plans-heading"><div><small>PEOPLE LEDGERS</small><h2>Who has to pay you?</h2></div><button onClick={openNewLedger}><Plus/> Add person</button></div>{!activePeopleLedgers.length?<Empty icon={<UsersRound/>} title="No people ledgers yet" text="Create an open ledger or set a total receivable with monthly installments."/>:<div className="people-list">{activePeopleLedgers.map(ledger=>{const outstanding=Math.max(0,ledgerOutstanding(ledger,ledgerEntries));const received=ledgerReceived(ledgerEntries,ledger.id);const principal=ledgerPrincipal(ledger,ledgerEntries);const next=installmentSchedule(ledger,ledgerEntries).find(row=>row.status!=="PAID");return <button className="person-card" key={ledger.id} onClick={()=>setSelectedLedger(ledger)}><span><UsersRound/></span><div><strong>{ledger.personName}</strong><small>{ledger.relation||"Personal ledger"}{next?` · Next ${formatMoney(next.amount-next.paid)} ${new Date(`${next.dueDate}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short"})}`:""}</small><div className="person-progress"><i style={{width:`${principal?Math.min(100,(received/principal)*100):0}%`}}/></div></div><b>{formatMoney(outstanding)}<small>outstanding</small></b><ChevronRight/></button>})}</div>}</>}
    </section>}
    ...
    {goalOpen&&<Modal close={()=>setGoalOpen(false)} title="New savings goal" subtitle="Reserve money for a future purchase or experience without treating it as an expense."><form className="manual-form" onSubmit={saveGoal}><label>Goal name<input autoFocus value={goalDraft.name} onChange={e=>setGoalDraft({...goalDraft,name:e.target.value})} placeholder="e.g. Family tour" required/></label><label>Target amount ({currencyPrefix(profile.currency)})<input type="number" min="1" step="0.01" inputMode="decimal" value={goalDraft.targetAmount} onChange={e=>setGoalDraft({...goalDraft,targetAmount:e.target.value})} placeholder="0" required/></label><label>Target date (optional)<input type="date" min={todayKey()} value={goalDraft.targetDate} onChange={e=>setGoalDraft({...goalDraft,targetDate:e.target.value})}/></label><label>Note (optional)<input value={goalDraft.note} onChange={e=>setGoalDraft({...goalDraft,note:e.target.value})} placeholder="What is this goal for?"/></label><p className="form-note"><Info/> Savings are earmarked inside your existing balance; they do not create money out.</p><button className="primary-button"><Target/> Create goal</button></form></Modal>}
    {goalEntryTarget&&<Modal close={()=>setGoalEntryTarget(null)} title={goalEntryDraft.direction==="ADD"?`Add to ${goalEntryTarget.name}`:`Withdraw from ${goalEntryTarget.name}`} subtitle={`Reserved ${formatMoney(goalSaved(savingsEntries,goalEntryTarget.id))} · Unreserved cash ${formatMoney(Math.max(0,totalBalance(transactions)-totalSaved))}`}><form className="manual-form" onSubmit={saveGoalEntry}><div className="direction-toggle"><button type="button" className={goalEntryDraft.direction==="ADD"?"selected in":""} onClick={()=>setGoalEntryDraft({...goalEntryDraft,direction:"ADD"})}>Add saving</button><button type="button" className={goalEntryDraft.direction==="WITHDRAW"?"selected out":""} onClick={()=>setGoalEntryDraft({...goalEntryDraft,direction:"WITHDRAW"})}>Withdraw</button></div><label>Amount ({currencyPrefix(profile.currency)})<input autoFocus type="number" min="0.01" step="0.01" inputMode="decimal" value={goalEntryDraft.amount} onChange={e=>setGoalEntryDraft({...goalEntryDraft,amount:e.target.value})} placeholder="0" required/></label><label>Date<input type="date" value={goalEntryDraft.date} onChange={e=>setGoalEntryDraft({...goalEntryDraft,date:e.target.value})} required/></label><label>Note (optional)<input value={goalEntryDraft.note} onChange={e=>setGoalEntryDraft({...goalEntryDraft,note:e.target.value})} placeholder="e.g. Monthly saving"/></label><button className="primary-button">Save goal entry</button></form></Modal>}
    {ledgerOpen&&<Modal close={()=>setLedgerOpen(false)} title="Create person ledger" subtitle="Use an open ledger for ongoing give-and-receive, or set a receivable target for an existing loan."><form className="manual-form" onSubmit={saveLedger}><label>Person name<input autoFocus value={ledgerDraft.personName} onChange={e=>setLedgerDraft({...ledgerDraft,personName:e.target.value})} placeholder="e.g. Ammi" required/></label><label>Relation (optional)<input value={ledgerDraft.relation} onChange={e=>setLedgerDraft({...ledgerDraft,relation:e.target.value})} placeholder="e.g. Mother, sister"/></label><div className="direction-toggle"><button type="button" className={ledgerDraft.mode==="OPEN"?"selected in":""} onClick={()=>setLedgerDraft({...ledgerDraft,mode:"OPEN",openingReceivable:""})}>Open ledger</button><button type="button" className={ledgerDraft.mode==="TARGET"?"selected in":""} onClick={()=>setLedgerDraft({...ledgerDraft,mode:"TARGET"})}>Target receivable</button></div>{ledgerDraft.mode==="TARGET"&&<label>Amount already receivable ({currencyPrefix(profile.currency)})<input type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerDraft.openingReceivable} onChange={e=>setLedgerDraft({...ledgerDraft,openingReceivable:e.target.value})} placeholder="0" required/></label>}<label>Monthly installment (optional)<input type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerDraft.installmentAmount} onChange={e=>setLedgerDraft({...ledgerDraft,installmentAmount:e.target.value})} placeholder="e.g. 10,000"/></label>{ledgerDraft.installmentAmount&&<label>First due date<input type="date" value={ledgerDraft.firstDueDate} onChange={e=>setLedgerDraft({...ledgerDraft,firstDueDate:e.target.value})} required/></label>}<label>Note (optional)<input value={ledgerDraft.note} onChange={e=>setLedgerDraft({...ledgerDraft,note:e.target.value})} placeholder="Loan or arrangement details"/></label><button className="primary-button"><UsersRound/> Create ledger</button></form></Modal>}
    {selectedLedger&&!ledgerEntryOpen&&!ledgerOpen&&<Modal close={()=>setSelectedLedger(null)} title={selectedLedger.personName} subtitle={`${selectedLedger.relation||"Personal receivable"} · ${selectedLedger.mode==="OPEN"?"Open cash in/out ledger":"Target receivable"}`}><div className="ledger-summary"><span><small>TOTAL</small><strong>{formatMoney(ledgerPrincipal(selectedLedger,ledgerEntries))}</strong></span><span><small>RECEIVED</small><strong>{formatMoney(ledgerReceived(ledgerEntries,selectedLedger.id))}</strong></span><span className="due"><small>OUTSTANDING</small><strong>{formatMoney(Math.max(0,ledgerOutstanding(selectedLedger,ledgerEntries)))}</strong></span></div><div className="ledger-action-grid"><button onClick={()=>openPersonEntry(selectedLedger,"LENT")}><ArrowUpRight/> Give money</button><button onClick={()=>openPersonEntry(selectedLedger,"RECEIVED")} disabled={ledgerOutstanding(selectedLedger,ledgerEntries)<=0}><ArrowDownLeft/> Receive payment</button></div>{installmentSchedule(selectedLedger,ledgerEntries).length>0&&<div className="schedule-box"><div><CalendarClock/><strong>Installments</strong></div>{installmentSchedule(selectedLedger,ledgerEntries).slice(0,6).map(row=><span key={row.number} className={row.status.toLowerCase()}><b>#{row.number} · {new Date(`${row.dueDate}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short"})}</b><small>{formatMoney(row.amount)} · {row.status}{row.paid?` · ${formatMoney(row.paid)} paid`:""}</small></span>)}</div>}<div className="ledger-history"><div className="mini-head"><h3>Cash in/out ledger</h3><small>{ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).length} entries</small></div>{ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).length===0?<p>No cash movement recorded yet.</p>:ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).sort((a,b)=>b.date.localeCompare(a.date)).map(entry=><article key={entry.id}><span className={entry.kind==="RECEIVED"?"in":"out"}>{entry.kind==="RECEIVED"?<ArrowDownLeft/>:<ArrowUpRight/>}</span><div><strong>{entry.note|| (entry.kind==="RECEIVED"?"Payment received":"Money given")}</strong><small>{new Date(`${entry.date}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short",year:"numeric"})} · {entry.walletId?walletNames[entry.walletId]:"Adjustment"}</small></div><b className={entry.kind==="RECEIVED"?"in":"out"}>{entry.kind==="RECEIVED"?"−":"+"}{formatMoney(entry.amount)}</b></article>)}</div><div className="pdf-actions"><button onClick={()=>void downloadLedger(selectedLedger)}><Download/> Save PDF</button><button onClick={()=>void shareLedger(selectedLedger)}><Share2/> Share on WhatsApp</button></div></Modal>}
    {ledgerEntryOpen&&selectedLedger&&<Modal close={()=>setLedgerEntryOpen(false)} title={ledgerEntryDraft.kind==="RECEIVED"?`Receive from ${selectedLedger.personName}`:`Give money to ${selectedLedger.personName}`} subtitle={ledgerEntryDraft.kind==="RECEIVED"?`Outstanding: ${formatMoney(Math.max(0,ledgerOutstanding(selectedLedger,ledgerEntries)))}`:"This will reduce the selected wallet and increase the receivable."}><form className="manual-form" onSubmit={savePersonEntry}><div className="direction-toggle"><button type="button" className={ledgerEntryDraft.kind==="LENT"?"selected out":""} onClick={()=>setLedgerEntryDraft({...ledgerEntryDraft,kind:"LENT"})}>Money given</button><button type="button" className={ledgerEntryDraft.kind==="RECEIVED"?"selected in":""} onClick={()=>setLedgerEntryDraft({...ledgerEntryDraft,kind:"RECEIVED"})}>Payment received</button></div><label>Wallet<select value={ledgerEntryDraft.walletId} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,walletId:e.target.value})} required>{availableWallets.map(wallet=><option key={wallet.id} value={wallet.id}>{walletLabel(wallet)} · {formatMoney(walletBalance(transactions,wallet.id))}</option>)}</select></label><label>Amount ({currencyPrefix(profile.currency)})<input autoFocus type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerEntryDraft.amount} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,amount:e.target.value})} placeholder="0" required/></label><label>Date<input type="date" value={ledgerEntryDraft.date} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,date:e.target.value})} required/></label><label>Note (optional)<input value={ledgerEntryDraft.note} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,note:e.target.value})} placeholder={ledgerEntryDraft.kind==="RECEIVED"?"e.g. August installment":"Purpose or details"}/></label><button className="primary-button">Save to wallet & ledger</button></form></Modal>}
