...
import {
  ArrowDownLeft, ArrowLeft, ArrowUpRight, Check,
  ChevronRight, Eye, EyeOff, FileSpreadsheet,
...
import { cashbookSpreadsheetPayload, transactionHeadersFor, transactionRows } from "./google-sheets";
...
import { goalProgress, goalSaved, installmentSchedule, ledgerBalanceSide, ledgerGiven, ledgerOutstanding, ledgerPrincipal, ledgerReceived, normalizePlanningState, PersonLedger, PersonLedgerEntry, savingsWalletDirection, SavingsEntry, SavingsGoal } from "./planning";
...
type Tab = "home" | "history" | "plans" | "settings";
...
type PlanningDeleteTarget = { kind:"savings"|"khaata"; id:string; label:string };
...
export default function CashApp(){
  ...
  const [ledgerDraft,setLedgerDraft]=useState({personName:"",relation:"",mode:"TARGET" as PersonLedger["mode"],openingSide:"RECEIVABLE" as "RECEIVABLE"|"PAYABLE",openingReceivable:"",installmentAmount:"",firstDueDate:"",note:""});
  ...
  const [planningDeleteTarget,setPlanningDeleteTarget]=useState<PlanningDeleteTarget|null>(null);
  ...
  const reportWalletTransactions=reportWalletId==="all"?transactions:transactions.filter(t=>t.walletId===reportWalletId);
  const filtered=reportWalletTransactions.filter(t=>t.description.toLowerCase().includes(query.toLowerCase())&&(!reportDates.from||t.date>=reportDates.from)&&(!reportDates.to||t.date<=reportDates.to));
  ...
  const totalPayable=activePeopleLedgers.reduce((sum,ledger)=>sum+Math.max(0,-ledgerOutstanding(ledger,ledgerEntries)),0);
  const khaataNet=totalReceivable-totalPayable;
  ...
  function openOpeningBalance(walletId=selectedWallet.id){
    const existing=currentOpeningBalance(transactions,walletId);
    if(walletId!==selectedWallet.id)selectWallet(walletId);
  ...
  function requestSavingsDelete(entry:SavingsEntry){const linked=entry.transactionId?transactions.find(t=>t.id===entry.transactionId):undefined;if(linked){setDeleteTarget(linked);return}setPlanningDeleteTarget({kind:"savings",id:entry.id,label:entry.note||"Savings entry"})}
  function requestKhaataDelete(entry:PersonLedgerEntry){const linked=entry.transactionId?transactions.find(t=>t.id===entry.transactionId):undefined;if(linked){setDeleteTarget(linked);return}setPlanningDeleteTarget({kind:"khaata",id:entry.id,label:entry.note||"Khaata entry"})}
  function confirmPlanningDelete(){if(!planningDeleteTarget)return;if(planningDeleteTarget.kind==="savings")setSavingsEntries(prev=>prev.filter(entry=>entry.id!==planningDeleteTarget.id));else setLedgerEntries(prev=>prev.filter(entry=>entry.id!==planningDeleteTarget.id));notify(`${planningDeleteTarget.kind==="savings"?"Savings":"Khaata"} entry deleted`);setPlanningDeleteTarget(null)}
  ...
  function openNewLedger(){setLedgerEditTarget(null);setLedgerDraft({personName:"",relation:"",mode:"OPEN",openingSide:"RECEIVABLE",openingReceivable:"",installmentAmount:"",firstDueDate:"",note:""});setLedgerOpen(true)}
  function openLedgerEdit(ledger:PersonLedger){setLedgerEditTarget(ledger);setLedgerDraft({personName:ledger.personName,relation:ledger.relation||"",mode:ledger.mode,openingSide:ledger.openingReceivable<0?"PAYABLE":"RECEIVABLE",openingReceivable:String(Math.abs(ledger.openingReceivable)||""),installmentAmount:String(ledger.installmentAmount||""),firstDueDate:ledger.firstDueDate||"",note:ledger.note||""});setLedgerOpen(true)}
  function saveLedger(e:FormEvent){
    e.preventDefault();const openingAmount=ledgerDraft.mode==="TARGET"?Number(ledgerDraft.openingReceivable):0;const openingReceivable=ledgerDraft.openingSide==="PAYABLE"?-openingAmount:openingAmount;const installmentAmount=Number(ledgerDraft.installmentAmount||0);
    if(!ledgerDraft.personName.trim()||(ledgerDraft.mode==="TARGET"&&!openingReceivable)){notify("Enter the person and starting amount");return}
  ...
  async function savePersonEntry(e:FormEvent){
    ...
    const baseEntries=ledgerEntryEditTarget?ledgerEntries.filter(entry=>entry.id!==ledgerEntryEditTarget.id):ledgerEntries;const baseTransactions=ledgerEntryEditTarget?.transactionId?transactions.filter(t=>t.id!==ledgerEntryEditTarget.transactionId):transactions;
    ...
    const description=ledgerEntryDraft.note.trim()||(direction==="IN"?`Money received from ${selectedLedger.personName}`:`Money given to ${selectedLedger.personName}`);
    const transaction:Transaction={id:transactionId,amount,description,direction,action:direction==="IN"?"Received":"Given",date:ledgerEntryDraft.date,time:now.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),source:"Manual",status:googleToken?"Pending":"Local",walletId:wallet.id,ledgerEntryId:entryId};
  ...
  async function shareLedger(ledger:PersonLedger){
    ...
    const balance=ledgerOutstanding(ledger,ledgerEntries);const {doc,filename}=await createLedgerPdf(ledger,ledgerEntries,profile,walletNames,{from:khaataDates.from,to:khaataDates.to});const file=ledgerPdfFile(doc,filename);const text=`${ledger.personName}'s Khaata · ${balance<0?"You will pay":"You will receive"} ${formatMoney(Math.abs(balance))}`;
  ...
  const reportTransactions=reportWalletTransactions.filter(t=>t.date>=reportDates.from&&t.date<=reportDates.to);
  const reportWalletLabel=reportWalletId==="all"?"All wallets":walletNames[reportWalletId]||"Wallet";
  ...
  async function downloadCashReport(){if(!cashReportRangeIsValid())return;const {doc,filename}=await createCashReportPdf(reportTransactions,profile,walletNames,reportDates.from,reportDates.to,reportWalletLabel);doc.save(filename);notify("Cash report PDF downloaded")}
  function downloadCashCsv(){if(!cashReportRangeIsValid())return;const blob=new Blob([cashReportCsv(reportTransactions,walletNames)],{type:"text/csv;charset=utf-8"});const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=`cash-report-${reportWalletId}-${reportDates.from}-to-${reportDates.to}.csv`;link.click();URL.revokeObjectURL(url);notify("Cash report CSV downloaded")}
  ...
  return <main className="phone-shell">
    {tab==="home"&&<section className="screen home-screen">
      ...
      <div className="home-plan-buttons"><button onClick={()=>{setPlanView("goals");setTab("plans")}}><PiggyBank/><span><strong>Savings</strong><small>{formatMoney(totalSaved)} saved</small></span><ChevronRight/></button><button onClick={()=>{setPlanView("people");setTab("plans")}}><UsersRound/><span><strong>Khaata</strong><small>{activePeopleLedgers.length} people · both directions</small></span><ChevronRight/></button></div>
    ...
    {tab==="history"&&<section className="screen"><Header title="History & reports" back/><div className="report-wallet-filter"><label>Wallet<select value={reportWalletId} onChange={e=>setReportWalletId(e.target.value)}><option value="all">All wallets</option>{profile.wallets.map(wallet=><option key={wallet.id} value={wallet.id}>{walletLabel(wallet)}{wallet.archived?" (archived)":""}</option>)}</select></label></div><div className="search-box"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Search ${reportWalletLabel.toLowerCase()}`}/></div><div className="date-filter"><label>From<input type="date" value={reportDates.from} onChange={e=>setReportDates({...reportDates,from:e.target.value})}/></label><label>To<input type="date" value={reportDates.to} onChange={e=>setReportDates({...reportDates,to:e.target.value})}/></label></div><div className="report-actions"><button onClick={()=>void downloadCashReport()}><Download/> Save PDF</button><button onClick={downloadCashCsv}><FileSpreadsheet/> Save CSV</button></div><div className="history-summary"><span>{filtered.length} entries · {reportWalletLabel}</span><strong>{formatSigned(totalBalance(filtered))}</strong></div><div className="history-list">{!filtered.length?<Empty icon={<History/>} title="No transactions found" text="Try another wallet, date range, or search."/>:filtered.map(t=><TransactionRow key={t.id} t={t} currency={profile.currency} walletName={walletNames[t.walletId]} showDate actions onEdit={()=>openEdit(t)} onDelete={isOpeningBalanceEntry(t)?undefined:()=>setDeleteTarget(t)}/>)}</div></section>}

    {tab==="plans"&&<section className="screen plans-screen"><Header title="Plans & ledgers" back/>
      <section className="plans-hero"><div><small>{planView==="goals"?"SAVINGS":"KHAATA"}</small><h2>{planView==="goals"?formatMoney(totalSaved):formatMoney(Math.abs(khaataNet))}</h2><p>{planView==="goals"?"Money currently held in savings":khaataNet>0?"Net amount you will receive":khaataNet<0?"Net amount you need to pay":"Khaata is balanced"}</p></div><span>{planView==="goals"?<PiggyBank/>:<HandCoins/>}</span></section>
      ...
      {planView==="goals"&&<><div className="plans-heading"><div><small>SAVINGS</small><h2>What are you saving for?</h2></div><button onClick={openNewGoal}><Plus/> Add goal</button></div>{!activeGoals.length?<Empty icon={<PiggyBank/>} title="Create your first savings goal" text="Track money moved from a wallet into savings for assets, shopping, tours, or anything important."/>:<div className="goal-list">{activeGoals.map(goal=>{const saved=goalSaved(savingsEntries,goal.id);const progress=goalProgress(goal,savingsEntries);const activity=savingsEntries.filter(entry=>entry.goalId===goal.id).sort((a,b)=>b.date.localeCompare(a.date));return <article className="goal-card" key={goal.id}><div className="goal-card-head"><span><Target/></span><button className="goal-name" onClick={()=>setSelectedGoal(goal)}><strong>{goal.name}</strong><small>{goal.targetDate?`Target ${new Date(`${goal.targetDate}T12:00:00`).toLocaleDateString("en-PK",{month:"short",year:"numeric"})}`:"No target date"}</small></button><button className="icon-edit" onClick={()=>openGoalEdit(goal)} aria-label={`Edit ${goal.name}`}><PenLine/></button></div><div className="goal-amount"><strong>{formatMoney(saved)}</strong><span>of {formatMoney(goal.targetAmount)}</span></div><div className="progress-track"><i style={{width:`${progress}%`}}/></div><div className="goal-activity"><div><strong>Recent activity</strong><button onClick={()=>setSelectedGoal(goal)}>{activity.length} {activity.length===1?"entry":"entries"} · View all</button></div>{activity.length===0?<p>No savings entries yet.</p>:activity.slice(0,2).map(entry=><article key={entry.id}><span className={entry.direction==="ADD"?"in":"out"}>{entry.direction==="ADD"?<ArrowDownLeft/>:<ArrowUpRight/>}</span><div><b>{entry.note||(entry.direction==="ADD"?"Saved from wallet":"Withdrawn to wallet")}</b><small>{new Date(`${entry.date}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short"})} · {entry.walletId?walletNames[entry.walletId]:"Older entry"}</small></div><strong className={entry.direction==="ADD"?"in":"out"}>{entry.direction==="ADD"?"+":"−"}{formatMoney(entry.amount)}</strong></article>)}</div><div className="goal-actions"><button onClick={()=>openGoalEntry(goal,"WITHDRAW")} disabled={saved<=0}>Withdraw to wallet</button><button onClick={()=>openGoalEntry(goal,"ADD")}><Plus/> Save from wallet</button></div></article>})}</div>}</>}
      {planView==="people"&&<><div className="plans-heading"><div><small>KHAATA</small><h2>Money with people</h2></div><button onClick={openNewLedger}><Plus/> Add Khaata</button></div>{!activePeopleLedgers.length?<Empty icon={<UsersRound/>} title="No Khaata yet" text="Track money they owe you or money you owe them."/>:<div className="people-list">{activePeopleLedgers.map(ledger=>{const balance=ledgerOutstanding(ledger,ledgerEntries);const side=ledgerBalanceSide(balance);const settled=ledger.openingReceivable<0?ledgerGiven(ledgerEntries,ledger.id):ledgerReceived(ledgerEntries,ledger.id);const principal=ledgerPrincipal(ledger,ledgerEntries);const next=installmentSchedule(ledger,ledgerEntries).find(row=>row.status!=="PAID");return <button className={`person-card ${side.toLowerCase()}`} key={ledger.id} onClick={()=>{setKhaataDates({from:"",to:todayKey()});setSelectedLedger(ledger)}}><span><UsersRound/></span><div><strong>{ledger.personName}</strong><small>{ledger.relation||"Khaata"}{next?` · Next ${formatMoney(next.amount-next.paid)} ${new Date(`${next.dueDate}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short"})}`:""}</small>{ledger.mode==="TARGET"&&<div className="person-progress"><i style={{width:`${principal?Math.min(100,(settled/principal)*100):0}%`}}/></div>}</div><b>{formatMoney(Math.abs(balance))}<small>{balance===0?"settled":side==="PAYABLE"?"you have to pay":"you will receive"}</small></b><ChevronRight/></button>})}</div>}</>}
    </section>}
    ...
    {tab==="settings"&&<section className="screen">
      ...
      <div className="settings-group"><small>OPENING BALANCES</small>
        {availableWallets.map(wallet=>{const opening=currentOpeningBalance(transactions,wallet.id);return <button key={wallet.id} onClick={()=>openOpeningBalance(wallet.id)}><span>{wallet.type==="bank"?<Landmark/>:<WalletCards/>}</span><div><strong>{opening?`Edit ${wallet.name} opening balance`:`Add ${wallet.name} opening balance`}</strong><small>{opening?`${formatMoney(opening.amount)} · ${new Date(`${opening.date}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short",year:"numeric"})}`:"Set once when this wallet is created"}</small></div><ChevronRight/></button>})}
      </div>
    ...
    {planningDeleteTarget&&<Modal close={()=>setPlanningDeleteTarget(null)} title={`Delete ${planningDeleteTarget.kind==="savings"?"savings":"Khaata"} entry?`} subtitle={planningDeleteTarget.label}><div className="delete-confirm"><p>This older entry has no linked wallet transaction. Only the planning record will be removed.</p><button className="danger-button" onClick={confirmPlanningDelete}><Trash2/> Delete entry</button><button className="secondary-button" onClick={()=>setPlanningDeleteTarget(null)}>Keep entry</button></div></Modal>}
    ...
    {selectedGoal&&!goalEntryTarget&&!goalOpen&&<Modal close={()=>setSelectedGoal(null)} title={selectedGoal.name} subtitle={`${formatMoney(goalSaved(savingsEntries,selectedGoal.id))} currently saved`}><div className="ledger-history"><div className="mini-head"><h3>Savings entries</h3><button onClick={()=>openGoalEdit(selectedGoal)}><PenLine/> Edit goal</button></div>{savingsEntries.filter(entry=>entry.goalId===selectedGoal.id).length===0?<p>No savings entries yet.</p>:savingsEntries.filter(entry=>entry.goalId===selectedGoal.id).sort((a,b)=>b.date.localeCompare(a.date)).map(entry=><article key={entry.id}><span className={entry.direction==="ADD"?"in":"out"}>{entry.direction==="ADD"?<ArrowDownLeft/>:<ArrowUpRight/>}</span><div><strong>{entry.note||(entry.direction==="ADD"?"Saved from wallet":"Withdrawn to wallet")}</strong><small>{new Date(`${entry.date}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short",year:"numeric"})} · {entry.walletId?walletNames[entry.walletId]:"Older entry"}</small></div><b className={entry.direction==="ADD"?"in":"out"}>{entry.direction==="ADD"?"+":"−"}{formatMoney(entry.amount)}</b><div className="entry-controls"><button onClick={()=>openGoalEntry(selectedGoal,entry.direction,entry)}><PenLine/> Edit</button><button className="delete" onClick={()=>requestSavingsDelete(entry)}><Trash2/> Delete</button></div></article>)}</div></Modal>}
    {ledgerOpen&&<Modal close={()=>{setLedgerOpen(false);setLedgerEditTarget(null)}} title={ledgerEditTarget?"Edit Khaata":"Create Khaata"} subtitle="Start at zero, or enter who owes whom. You can give or receive money as the first entry."><form className="manual-form" onSubmit={saveLedger}><label>Person name<input autoFocus value={ledgerDraft.personName} onChange={e=>setLedgerDraft({...ledgerDraft,personName:e.target.value})} placeholder="e.g. Ammi" required/></label><label>Relation (optional)<input value={ledgerDraft.relation} onChange={e=>setLedgerDraft({...ledgerDraft,relation:e.target.value})} placeholder="e.g. Mother, sister"/></label><div className="direction-toggle"><button type="button" className={ledgerDraft.mode==="OPEN"?"selected in":""} onClick={()=>setLedgerDraft({...ledgerDraft,mode:"OPEN",openingReceivable:"",installmentAmount:"",firstDueDate:""})}>Start at zero</button><button type="button" className={ledgerDraft.mode==="TARGET"?"selected in":""} onClick={()=>setLedgerDraft({...ledgerDraft,mode:"TARGET"})}>Starting balance</button></div>{ledgerDraft.mode==="TARGET"&&<><div className="direction-toggle balance-side"><button type="button" className={ledgerDraft.openingSide==="RECEIVABLE"?"selected in":""} onClick={()=>setLedgerDraft({...ledgerDraft,openingSide:"RECEIVABLE"})}>They owe me</button><button type="button" className={ledgerDraft.openingSide==="PAYABLE"?"selected out":""} onClick={()=>setLedgerDraft({...ledgerDraft,openingSide:"PAYABLE"})}>I owe them</button></div><label>Starting amount ({currencyPrefix(profile.currency)})<input type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerDraft.openingReceivable} onChange={e=>setLedgerDraft({...ledgerDraft,openingReceivable:e.target.value})} placeholder="0" required/></label><label>Monthly installment (optional)<input type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerDraft.installmentAmount} onChange={e=>setLedgerDraft({...ledgerDraft,installmentAmount:e.target.value})} placeholder="e.g. 10,000"/></label>{ledgerDraft.installmentAmount&&<label>First due date<input type="date" value={ledgerDraft.firstDueDate} onChange={e=>setLedgerDraft({...ledgerDraft,firstDueDate:e.target.value})} required/></label>}</>}<label>Note (optional)<input value={ledgerDraft.note} onChange={e=>setLedgerDraft({...ledgerDraft,note:e.target.value})} placeholder="Loan or arrangement details"/></label><button className="primary-button"><UsersRound/> {ledgerEditTarget?"Save Khaata changes":"Create Khaata"}</button></form></Modal>}
    {selectedLedger&&!ledgerEntryOpen&&!ledgerOpen&&<Modal close={()=>setSelectedLedger(null)} title={selectedLedger.personName} subtitle={`${selectedLedger.relation||"Khaata"} · ${selectedLedger.mode==="OPEN"?"Started at zero":selectedLedger.openingReceivable<0?"You owed them at start":"They owed you at start"}`}>
      <button className="modal-edit-button" onClick={()=>openLedgerEdit(selectedLedger)}><PenLine/> Edit Khaata & starting balance</button>
      <div className="ledger-summary"><span><small>GIVEN</small><strong>{formatMoney(ledgerGiven(ledgerEntries,selectedLedger.id))}</strong></span><span><small>RECEIVED</small><strong>{formatMoney(ledgerReceived(ledgerEntries,selectedLedger.id))}</strong></span><span className={ledgerOutstanding(selectedLedger,ledgerEntries)<0?"payable":"due"}><small>{ledgerOutstanding(selectedLedger,ledgerEntries)<0?"YOU WILL PAY":"YOU WILL RECEIVE"}</small><strong>{formatMoney(Math.abs(ledgerOutstanding(selectedLedger,ledgerEntries)))}</strong></span></div>
      <div className="ledger-action-grid"><button onClick={()=>openPersonEntry(selectedLedger,"LENT")}><ArrowUpRight/> Give money</button><button onClick={()=>openPersonEntry(selectedLedger,"RECEIVED")}><ArrowDownLeft/> Receive money</button></div>
      ...
      <div className="ledger-history"><div className="mini-head"><h3>Khaata entries</h3><small>{ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).length} entries</small></div>{ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).length===0?<p>No cash movement yet. You can give or receive money as the first entry.</p>:ledgerEntries.filter(entry=>entry.ledgerId===selectedLedger.id).sort((a,b)=>b.date.localeCompare(a.date)).map(entry=><article key={entry.id}><span className={entry.kind==="RECEIVED"?"in":"out"}>{entry.kind==="RECEIVED"?<ArrowDownLeft/>:<ArrowUpRight/>}</span><div><strong>{entry.note||(entry.kind==="RECEIVED"?"Money received":"Money given")}</strong><small>{new Date(`${entry.date}T12:00:00`).toLocaleDateString("en-PK",{day:"numeric",month:"short",year:"numeric"})} · {entry.walletId?walletNames[entry.walletId]:"Adjustment"}</small></div><b className={entry.kind==="RECEIVED"?"in":"out"}>{entry.kind==="RECEIVED"?"−":"+"}{formatMoney(entry.amount)}</b><div className="entry-controls"><button onClick={()=>openPersonEntry(selectedLedger,entry.kind,entry)}><PenLine/> Edit</button><button className="delete" onClick={()=>requestKhaataDelete(entry)}><Trash2/> Delete</button></div></article>)}</div>
    ...
    {ledgerEntryOpen&&selectedLedger&&<Modal close={()=>{setLedgerEntryOpen(false);setLedgerEntryEditTarget(null)}} title={ledgerEntryEditTarget?"Edit Khaata entry":ledgerEntryDraft.kind==="RECEIVED"?`Receive money from ${selectedLedger.personName}`:`Give money to ${selectedLedger.personName}`} subtitle={ledgerEntryDraft.kind==="RECEIVED"?"This adds cash to the selected wallet. If they do not already owe you, it becomes money you owe them.":"This reduces the selected wallet. It settles what you owe or becomes money they owe you."}><form className="manual-form" onSubmit={savePersonEntry}><div className="direction-toggle"><button type="button" className={ledgerEntryDraft.kind==="LENT"?"selected out":""} onClick={()=>setLedgerEntryDraft({...ledgerEntryDraft,kind:"LENT"})}>Give money</button><button type="button" className={ledgerEntryDraft.kind==="RECEIVED"?"selected in":""} onClick={()=>setLedgerEntryDraft({...ledgerEntryDraft,kind:"RECEIVED"})}>Receive money</button></div><label>Wallet<select value={ledgerEntryDraft.walletId} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,walletId:e.target.value})} required>{availableWallets.map(wallet=><option key={wallet.id} value={wallet.id}>{walletLabel(wallet)} · {formatMoney(walletBalance(transactions,wallet.id))}</option>)}</select></label><label>Amount ({currencyPrefix(profile.currency)})<input autoFocus type="number" min="0.01" step="0.01" inputMode="decimal" value={ledgerEntryDraft.amount} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,amount:e.target.value})} placeholder="0" required/></label><label>Date<input type="date" value={ledgerEntryDraft.date} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,date:e.target.value})} required/></label><label>Note (optional)<input value={ledgerEntryDraft.note} onChange={e=>setLedgerEntryDraft({...ledgerEntryDraft,note:e.target.value})} placeholder={ledgerEntryDraft.kind==="RECEIVED"?"e.g. money borrowed or installment received":"e.g. loan or repayment"}/></label><button className="primary-button">{ledgerEntryEditTarget?"Save Khaata entry changes":"Save to wallet & Khaata"}</button></form></Modal>}
